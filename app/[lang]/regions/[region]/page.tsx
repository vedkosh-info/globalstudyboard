...
export function generateStaticParams() {
  // Generate params for both en and hi locales × 8 regions
  const locales = ['en', 'hi'];
  return locales.flatMap((lang) =>
    REGION_SLUGS.map((region) => ({ lang, region }))
  );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.globalstudyboard.com/regions/${r.slug}`,
      languages: {
        en: `https://www.globalstudyboard.com/regions/${r.slug}`,
        hi: `https://www.globalstudyboard.com/hi/regions/${r.slug}`,
        'x-default': `https://www.globalstudyboard.com/regions/${r.slug}`,
      },
    },
    openGraph: {
      type: 'website',
